import argparse
# from my_work.pmsmodel.laplacian import LapLoss
import os
...
from datasets.pms_dataset import *
from pmsmodel import *
from utils import *
import gc
import sys
import datetime
...
# dataset, dataloader
# MVSDataset = PMSDataset()
# if args.dataset == 'dtu_yao':
train_dataset = PMSDataset(args.trainpath, "train")
test_dataset = PMSDataset(args.valpath, "val")
...
def train():
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            do_summary_image = global_step % (args.summary_freq) == 0
...
def train_sample(sample, detailed_summary=False):
    ...
    warped_srcs = outputs["warped_srcs"]

    optimizer.zero_grad()
    loss = 0
    # ratio = [0.5, 0.25, 0.15, 0.1]
    ratio = [0.7, 0.15, 0.1, 0.05]
    for l in reversed(range(0, 4)):
        loss0 = model_loss(warped_srcs[f"stage_{l}"]['warped_src0'], sample_cuda["imgs"][f'stage_{l}'][:, 0], 5-l).mean()
        loss1 = model_loss(warped_srcs[f"stage_{l}"]['warped_src1'], sample_cuda["imgs"][f'stage_{l}'][:, 0], 5-l).mean()
        loss += (loss0 + loss1) * ratio[l]
    # loss = model_loss(warped_srcs, sample_cuda["imgs"]['stage_0'][:, 0])
    # loss = Variable(loss, requires_grad=True)
    loss.retain_grad()
    loss.backward()
    # print('=====loss variable=====', loss.is_leaf, loss.requires_grad, loss.grad, loss.grad_fn)
    optimizer.step()
    # gt = sample["imgs"]['stage_0'][:, 0]
    ...
    image_outputs = {"depth_refined_stage_0": depth_est['stage_0'],
                    # "depth_gt_stage_0": depth_gt['stage_0'] * mask['stage_0'],
                    "depth_patchmatch_stage_1": depth_patchmatch['stage_1'][-1],
                    "depth_patchmatch_stage_2": depth_patchmatch['stage_2'][-1],
                    "depth_patchmatch_stage_3": depth_patchmatch['stage_3'][-1],
                    "ref_img": sample["imgs"]['stage_0'][:, 0],
                    "warped_src0": warped_srcs['stage_0']['warped_src0'],
                    "warped_src1": warped_srcs['stage_0']['warped_src1'],
                     }
    # if detailed_summary:
