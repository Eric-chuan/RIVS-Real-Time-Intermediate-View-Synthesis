...
from .module import ConvBnReLU, depth_regression, image_differentiable_warping
from .PMSNet import PatchMatch
...
class PatchmatchNet(nn.Module):
    ...
    def forward(
        ...
        imgs_3 = torch.unbind(imgs["stage_3"], 1)                   #different resolution: 0 is the origion one
        del imgs

        self.imgs_0_ref, self.imgs_0_srcs = imgs_0[0], imgs_0[1:]
        self.imgs_1_ref, self.imgs_1_srcs = imgs_1[0], imgs_1[1:]
        self.imgs_2_ref, self.imgs_2_srcs = imgs_2[0], imgs_2[1:]
        self.imgs_3_ref, self.imgs_3_srcs = imgs_3[0], imgs_3[1:]
        ...
        for l in reversed(range(1, self.stages)):
            ...
            warped_srcs_l = {}
            imgs_l_src = getattr(self, f"imgs_{l}_srcs")
            for j in range(len(imgs_l_src)):        #should be two
                warped_srcs_l[f"warped_src{j}"] = image_differentiable_warping(imgs_l_src[j], src_projs[j], ref_proj, depth)
            warped_srcs[f"stage_{l}"] = warped_srcs_l
        ...
        warped_srcs["stage_0"] = {}
        imgs_0_src = self.imgs_0_srcs
        for j in range(len(imgs_0_src)):        #should be two
            warped_srcs["stage_0"][f"warped_src{j}"] = image_differentiable_warping(imgs_0_src[j], self.proj_matrices_0[1+j], self.proj_matrices_0[0], depth)
...
def warp_loss(
            warped_srcs: Dict[str, torch.Tensor],
            ref: torch.Tensor,
    ) -> torch.Tensor:
        """Patchmatch Net loss function
        ...
        #     images.append(torch.unbind(imgs[f"stage_{i}"], 1))                  #W//8
        #     proj_matrix.append(torch.unbind(proj_matrices[f"stage_{i}"].float(), 1))
        # del imgs
        # del proj_matrices
        lap = LapLoss(3)
        loss0 = lap(warped_srcs['warped_src0'], ref).mean()
        loss1 = lap(warped_srcs['warped_src1'], ref).mean()
        loss = loss0 + loss1
        return loss