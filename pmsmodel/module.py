...
def image_differentiable_warping(
    src: torch.Tensor, src_proj: torch.Tensor, ref_proj: torch.Tensor, depth: torch.Tensor
):
    """Differentiable homography-based warping, implemented in Pytorch.
    ...
    y, x = torch.meshgrid(
        [
            torch.arange(0, height, dtype=torch.float32, device=src.device),
            torch.arange(0, width, dtype=torch.float32, device=src.device),
        ]
    )
    y, x = y.contiguous(), x.contiguous()
    y, x = y.view(height * width), x.view(height * width)
    ...
    rot_depth_xyz = rot_xyz * depth.view(
    ...
    proj_x_normalized = proj_xy[:, 0, :] / ((width - 1) / 2) - 1  # [B, H*W]
    proj_y_normalized = proj_xy[:, 1, :] / ((height - 1) / 2) - 1
    ...
    warped_src = warped_src.view(batch, channels, height, width)
