...
def set_id_grid(depth):
    ...
    pixel_coords = torch.stack((j_range, i_range, ones), dim=1).to(device)  # [1, 3, H, W]
...
def inverse_warp(img, depth, pose_mat, mode='bilinear', padding_mode='border'):
    """
    ...
    intrinsics = np.array(
            ...
            0., 718.9, 128, \
            0., 0., 1.]).reshape((3, 3))
    intrinsics = torch.Tensor(intrinsics.astype(np.float32)).to(device).unsqueeze(0)
    ...
    proj_cam_to_src_pixel = intrinsics.bmm(pose_mat.squeeze(1)[:,:3,:])  # [B, 3, 4]

    src_pixel_coords,mask = cam2pixel(cam_coords, proj_cam_to_src_pixel[:,:,:3], proj_cam_to_src_pixel[:,:,-1:], padding_mode)  # [B,H,W,2]
    projected_img = F.grid_sample(img, src_pixel_coords, mode=mode, padding_mode=padding_mode,  align_corners=True)

    return projected_img,src_pixel_coords,mask

def forward_warp(depth, pose_mat,mode='bilinear', padding_mode='border'):
    i = 0